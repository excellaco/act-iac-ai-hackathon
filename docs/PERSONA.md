...
Maya's primary goal when using Parcela is to **build a defensible, data-backed case for regulatory change** that she can present to decision-makers without it being dismissed as opinion or advocacy.
...
**It's hard to make the regulatory impact concrete.** Maya knows intuitively that parking minimums drive up construction costs, but translating that into a specific dollar figure she can defend to a skeptical supervisor is difficult without access to proprietary cost modeling tools.
...
- **Search-first entry:** Maya knows her jurisdiction. She should never have to navigate a national map to find Fairfax County.
...
- **Descriptive, not prescriptive:** The RIS must never appear to recommend a policy direction. Maya works in a politically sensitive environment and needs a tool that measures without advocating.
...
- **No jargon she didn't bring:** Terms like DCI, DCOI, and CRP should be explained in plain language in the UI. Maya knows planning terminology but not the internal scoring framework.
