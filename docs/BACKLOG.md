...
| E2-2 | As a developer, I need an LLM prompt that extracts height limits so that DCI can be computed | Returns height limit in feet + confidence tier; ≥80% accuracy against gold fixtures (E2-0) | P0 | 2 |
| E2-3 | As a developer, I need an LLM prompt that extracts density limits (units/acre) so that DCI can be computed | Returns units/acre figure + confidence tier; ≥80% accuracy against gold fixtures (E2-0) | P0 | 2 |
| E2-4 | As a developer, I need an LLM prompt that extracts parking minimums so that DCOI can be computed | Returns spaces/unit figure + confidence tier; ≥80% accuracy against gold fixtures (E2-0) | P0 | 2 |
