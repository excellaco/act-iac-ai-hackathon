...
- **Software Bill of Materials (SBOM)** — generated automatically on every deploy using [Syft](https://github.com/anchore/syft) against the deployed container image in CycloneDX JSON format (covers base OS packages, npm dependencies, and build-time additions); downloadable from the [Actions tab](https://github.com/excellaco/act-iac-ai-hackathon/actions/workflows/ci-cd.yml) under **Artifacts** on the latest CI/CD run
